...
export default {
    ...
    mounted() {
        ...
        this.myScroll = new iScroll(this.$refs.classgoodscomponent,{
            preventDefault: false,
			probeType: 3,
        });
        var ot = this.$refs.console.offsetTop * -1;
        var size = 0;
        this.myScroll.on("scroll",function() {
            ...
            size = this.y >= 100 ? 100 : (this.y <= -100 ? -100 : this.y);
            that.$refs.pulldown.$el.style.transform = "scale("+ size/100 +")";
        })
        this.$refs.main.addEventListener("touchend",function(e){
            if(that.myScroll.y >= 100){   //重置所有数据
                that.level2 = [];  //二级分类
                that.brandid = [];  //品牌制造商id
                that.price1 = "";  //最低价格
                that.price2 = ""; //最高价格
                that.getData(that.filterNum);
                that.getLevel2Data();
            }
        })
        ...
        getData(filterNum) {
            ...
            this.$http.post(this.apiUrl,{
                ...
                if(data.ztm == 1) {
                    ...
                    setTimeout(() => {
                        this.myScroll.refresh();
                        //滚动区域高度更新后重新检查控制栏是否显示
                        if(this.myScroll.y <= this.$refs.console.offsetTop * -1) {
                            this.consoleIsShow = true;
                        }else {
                            this.consoleIsShow = false;
                        }
                    },300)
                }else {

<template>
    <div class="zcwriteorder-page">
        <ComHeaderbar text="填写订单"></ComHeaderbar>
        <div class="main" v-if="goodsData.goodslist">
            ...
            <yd-cell-group>
                ...
                <yd-cell-item type="a" href="javascript:;">
                    ...
                    <span slot="right">¥{{ freight.toFixed(2) }}</span>
                </yd-cell-item>
            </yd-cell-group>
            <ul class="goods-list">
        ...
        <footer v-if="goodsData.goodslist">
            <div>
                应付：¥{{ total.toFixed(2) }}
            </div>
    ...
    methods: {
        payment() {
            ...
            this.$http.post(this.apiUrl,{
                code: "CrowdFundingAddOrder",
                addressid: this.goodsData.address.id,
                goodsid: this.goodsData.goodslist[0].goodsid,
                number: this.goodsData.goodslist[0].number,
                specificationsid: this.goodsData.goodslist[0].specificationsid,
            }).then((data) => {
                console.log(data)
                if(data.ztm == 1) {
                    this.$router.push('payment?iszc=1&back=1&oid=' + data.ordersn);
                }else {
                    this.$dialog.toast({
                        mes: data.msg,
                        icon: "error"
